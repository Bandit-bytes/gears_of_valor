...
public class ReaperArmorItem extends ArmorItem {
    ...
    private static final String LAST_HEAL_TIME_TAG = "ReaperLastHealTime";
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity entity, int slotId, boolean isSelected) {
        if (!level.isClientSide() && entity instanceof Player player) {
            if (hasFullSet(player)) {
                ...
                ).forEach(enemy -> enemy.addEffect(new MobEffectInstance(MobEffects.WEAKNESS, 60, 0, false, false)));

                ItemStack chest = player.getItemBySlot(EquipmentSlot.CHEST);
                if (player.getAttackStrengthScale(1.0F) > 0.9F && player.getLastHurtMob() != null) {
                    long currentTime = level.getGameTime();
                    long lastHeal = chest.hasTag() ? chest.getTag().getLong(LAST_HEAL_TIME_TAG) : 0L;

                    if (currentTime - lastHeal >= 40) {
                        player.heal(2.0F);
                        chest.getOrCreateTag().putLong(LAST_HEAL_TIME_TAG, currentTime);
                    }
                }
            }
        }
